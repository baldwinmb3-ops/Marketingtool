...
function sanitizeBookingRow(input, defaults = {}) {
  ...
  return {
    ...
    guestEmail: normalizeEmail(src.guestEmail || base.guestEmail || ''),
    tourDate: String(src.tourDate || base.tourDate || '').trim(),
    ...
    createdByName: String(src.createdByName || src.created_by_name || base.createdByName || '').trim(),
    createdByWwid: normalizeWwid(src.createdByWwid || src.created_by_wwid || base.createdByWwid || ''),
